<template>
  <div class="container-fluid p-0 px-md-2">
    <div :class="isMobile ? 'game-list-menu-mobile' : 'fb-scroll game-list-menu'">
      <fb-select-sport
      ...
      <fb-2-select-page-slider
        v-if="getCurrentLayoutStyle === 'FB2' && currentSettings && isMobile"
        :landing-style="getCurrentApplicationType"
        :font="currentSettings.font"
      />
      <fb-2-sports-select
        v-if="getCurrentLayoutStyle === 'FB2' && currentSettings && isMobile"
        :all-sports-data="sportsData"
        :current-sport="currentSport"
        @changeSport="changeSport($event)"
      />
      <template v-for="(sport, i) in getInplayEventsGroupedBySportAndCountry">
        <div
          v-if="!currentSport || currentSport === +sport.sport_id"
          :key="'games_inplay_group_' + i"
        ...
        >
          <div class="header-sport w-100 d-flex justify-content-between align-items-center">
            <div class="d-flex align-items-center py-2 px-1">
              ...
              <span class="sport-title text-white text-md-background fut-color-dynamic">{{ getSportName(sport, getMetadataInplaySports) }}</span>
            </div>
          </div>
          <template v-for="(location, j) in sport.locations">
            <fb-inplay-game-list
              v-if="currentSettings"
              :key="'games_listing_component_' + j"
...
export default {
  ...
  computed: {
    ...
    sportsData () {
      if (this.getMetadataInplaySports && this.getMetadataInplaySports.length) {
        return this.getMetadataInplaySports.map((item) => {
          return {
            name: item.name,
            id: item.id,
            games: item.total
  ...
  created () {
    ...
    this.loading = true
    this.fetchInplayEvents().then(() => {
      if (this.isMobile && this.getCurrentLayoutStyle === 'FB2') {
        this.currentSport = this.getInplayEventsGroupedBySportAndCountry[0].sport_id
      }
    }).finally(() => {
      this.loading = false
    })
