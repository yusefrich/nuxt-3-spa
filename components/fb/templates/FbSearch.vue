<template>
  <div>
    <fb-sections-heading
      ...
      route="/sports"
    />
    <template v-for="(group, i) in getPreMatchSeachEvents" :key="'searched_sports_' + i">
      <template v-for="(sport, j) in group.sport" :key="'searched_sports_' + i + '_' + j">
        <template v-for="(league, k) in sport.leagues" :key="'searched_sports_' + i + '_' + j + '_' + k">
          <fb-games-list
