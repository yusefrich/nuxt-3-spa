<template>
  <div class="games-list-container-spacing mt-md-3 mt-2 overflow-auto">
    ...
    <table class="odds-table py-1">
      <template v-for="(betGroup, indexNb) in events" :key="betGroup.name">
        <thead v-if="indexNb === 0" class="header">
          <tr class="text-center d-none d-md-table-row">
            <td />
            <td class="d-none d-md-table-cell" />
            <template v-for="(oddTitle, oddTitleIndex) in getOddTitleHeading(betGroup.games)" :key="'heading_' + oddTitleIndex">
              <td :class="{'d-none d-md-table-cell': oddTitleIndex > 0}" class="text-center" :colspan="oddTitle.colspan">
          ...
          <tr class="text-center">
            <td />
            <td class="d-none d-md-table-cell" />
            <template v-for="(oddTitle, oddTitleIndex) in getOddTitleHeading(betGroup.games)" :key="'heading__' + oddTitleIndex">
        ...
        <template v-for="(game, gameIndex) in betGroup.games" :key="game.id">
          <tbody>
            <tr class="tr-odds" :class="{ 'bg-dark-font': font === 'dark' }">
              ...
              <template v-for="(oddsGroup, index) in gameOdds(game.odds, betGroup.games, gameIndex - 1).list" :key="'odd_slug_j__'+index">
