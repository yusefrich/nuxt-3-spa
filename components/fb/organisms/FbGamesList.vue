      ...
      <template v-for="(betGroup, indexNb) in events">
        <thead v-if="indexNb === 0" :key="betGroup.name" class="header">
          <tr class="text-center d-none d-md-table-row">
            ...
            >
              <td
                :class="{'d-none d-md-table-cell': oddTitleIndex > 0}"
                class="text-center"
                :colspan="oddTitle.colspan"
              >
                <small
                  class="market-title fut-color-dynamic"
                  :style="'max-width:' + (oddTitle.oddheading ? oddTitle.oddheading.length * 43 : 43) + 'px;'"
                >
                  {{ oddTitle.title }}
                </small>
              </td>
          ...
          <tr class="text-center">
            <td />
            <td class="d-none d-md-table-cell" />
            <template v-if="!landingStyle || !isMobile">
              <template v-for="(oddTitle, oddTitleIndex) in getOddTitleHeading(betGroup.games)">
                <td
                  v-for="(heads, hIndex) in oddTitle.oddheading"
                  :key="'heading_' + oddTitleIndex + '_' + hIndex"
                  :class="{'d-none d-md-table-cell': oddTitleIndex > 0}"
                  class="text-center pt-2 pt-md-0"
                >
        ...
        <template v-for="(game, gameIndex) in betGroup.games" :key="game.id">
          <tbody>
            <tr class="tr-odds" :class="{ 'bg-dark-font': font === 'dark' }">
              ...
              <td class="event-name">
                <fb-fut-button
                  v-if="game.event"
                  :to="landingStyle && isMobile ? '' : game.sport_id ? '/sport/'+game.sport_id+'/evento/'+game.slug : eventsSport !== 18 && eventsSport !== 9 && eventsSport !== 14 ? '/sport/'+eventsSport+'/evento/'+game.slug : '#'"
                ...
                >
                  ...
                  <div class="d-md-none text-start d-flex justify-content-between align-items-center">
                    ...
                    <template v-for="(oddsGroup, index) in gameOdds(game.odds, betGroup.games, gameIndex - 1).list">
                      <template
                        v-if="(settings && settings.both_score === 0 && index === 0) || (settings && settings.both_score === 1)"
                      >
                        <template
                          v-for="(odd, j) in oddsGroup.odds"
                        >
                          <div
                            v-if="index < 1 && (odd.desc.toLowerCase() === game.home_team.toLowerCase() || odd.header.toLowerCase() === game.home_team.toLowerCase() || odd.name.toLowerCase() === game.home_team.toLowerCase())"
                            ...
                            :class="[
                              ...
                              'click-bet',
                              !odd.odds||odd.odds<=0||odd.odds==1?'click-bet-disabled':''
                            ]"
                            class="pe-3 mb-1"
                  ...
                  <div class="d-md-none text-start d-flex justify-content-between align-items-center">
                    ...
                    <template v-for="(oddsGroup, index) in gameOdds(game.odds, betGroup.games, gameIndex - 1).list">
                      ...
                      >
                        ...
                        >
                          <div
                            v-if="index < 1 && (odd.desc.toLowerCase() === game.away_team.toLowerCase() || odd.header.toLowerCase() === game.away_team.toLowerCase() || odd.name.toLowerCase() === game.away_team.toLowerCase())"
                            :key="'odd_slug_j_'+j+'_'+index"
                            :class="[
                              'text-white',
                              'fut-color-dynamic',
                  ...
                  <div class="d-flex d-md-none justify-content-between mt-1 w-fb-custom">
                    ...
                    <span v-if="!landingStyle" class="text-white me-1">+{{ game.odds_total }} <font-awesome-icon class="text-primary" :icon="['fas', 'caret-right']" /></span>
                  </div>
                </fb-fut-button>
              </td>
              <template v-if="!landingStyle || !isMobile">
        ...
        if (e.odds && Array.isArray(e.odds)) {
          e.odds.forEach((f) => {
            if (title.findIndex(obj => (obj.title === f.name) || (f.name === 'Resultado Final' && obj.title === '1 X 2')) === -1) {
              const data = {
                title: f.name === 'Resultado Final' ? '1 X 2' : f.name,
                colspan: f.odds.length,
                oddheading: []
              }
