<template>
  <div id="sidebarListSports">
    ...
    <div class="d-flex mob-menu-container">
      <!-- <h1 class="text-white">teste ok ok</h1> -->
      <fb-fut-button class="mob-menu-container-btn">
        Pré-Jogo
      </fb-fut-button>
    </div>
    <div>
            ...
            <li v-for="(item, idx2) in sport.country" :key="`count_${item.id}_${idx2}_${customId}`">
              <template v-if="item.link">
                ...
                >
                  ...
                  <span v-if="selectedGroupIds && checkId(selectedGroupIds, item.cc + '')" class="text-success check-group"><font-awesome-icon :icon="['fas', 'check']" /></span>
...
export default {
  ...
  methods: {
    ...
    changeTimeFilter () {
      ...
      this.$root.$emit('fetchTimeFilter', filterInHours)
    }
  }
}
</script>
