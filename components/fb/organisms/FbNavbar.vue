<template>
  <nav id="page-top" class="d-print-none">
    ...
    >
      <div class="d-flex">
          ...
          >
            ...
            <!-- <template v-else>
              <transition name="fade">
                <font-awesome-icon class="me-1" :icon="['fas', 'times']" />
              </transition>
            </template> -->
          </fb-fut-button>
        ...
        <ul class="nav d-md-flex d-none">
          <li v-if="productRedirect && loggedInUser" class="nav-item">
            <fb-fut-button
              class="nav-link d-flex align-items-center text-white fut-color-dynamic"
          ...
          >
            <fb-fut-button
              class="nav-link d-flex align-items-center fut-color-dynamic"
    ...
    <!-- Login Modal -->
    <fb-modal v-if="!loggedInUser" :open="modalLogin" @onClose="()=>{ modalLogin = false; }">
      ...
      <div class="text-center justify-content-center mt-12">
        ...
        >
          ...
          <div class="d-flex justify-content-between mt-3">
            ...
            <div>
              <fb-fut-button class="text-end forgot-pass-link w-100 fut-color-dynamic" to="/login/novaSenha">
    ...
    <div v-if="currentSettings" class="second-navbar container-fluid py-1 d-md-flex d-none align-items-center justify-content-between" :class="{'bg-secondary': currentSettings.font === 'dark'}">
      ...
      <div class="d-flex align-items-center mr-2">
        <span class="text-white font-size-time mr-20">
          <!-- <small>{{ formattedDateDay() }}, {{ timestamp }}</small> -->
          <small>{{ formattedDateDay() }}</small>
        </span>
        <!-- <div v-if="loggedInUser">
          <a
            class="nav-link text-primary text-user-dashboard p-0 px-3"
            :href="localePath('/my-bets')"
          >
  ...
  computed: {
    ...
    navItems () {
      ...
      if (!this.applicationType('sports') || this.applicationType('casino')) {
        items.push(
          ...
          { link: '/casino?cat=live', text: this.$t('i18n_cassino_ao_vivo'), icon: '/FbIcons/live_casino.svg' }
        )
      }
    ...
    mobileNavItems () {
      ...
      items.main = navItemsWithSports.filter(item =>
        ...
        (
          (item.link === '/casino' || item.link === '/casino?cat=live') &&
