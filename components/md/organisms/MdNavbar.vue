  ...
  <nav v-if="currentSettings" class="md-navbar transition">
    <div class="left-side content-side-scroll">
      ...
      <div v-if="!sidebar || sidebarClosed" class="side-logo transition">
        <NuxtLink :to="localePath('/')" class="sidebar-brand">
          <img
            class="sidebar-logo ms-1 w-100"
            :class="{'sidebar-brand-bg-color': currentSettings.background_logo === 1 || currentSettings.background_logo === null}"
    ...
    <div class="right-side">
      ...
      <div v-else class="d-flex">
        <NuxtLink :to="localePath('/login')" class="login-btn mx-2 transition">
          Login
        </NuxtLink>
        <NuxtLink :to="localePath('/register')" class="register-btn transition">
          {{ $t('i18n_registrar') }}
        </NuxtLink>
      </div>
    </div>
...
import MdUserDropdown from '@/components/md/molecules/MdUserDropdown'
    ...
    navItems () {
      ...
      if (!this.applicationType('sports') || this.applicationType('casino')) {
        items.push(
          ...
          { link: '/casino-aovivo', text: this.$t('i18n_cassino_ao_vivo'), icon: ['fas', 'coins'] }
        )
      }
