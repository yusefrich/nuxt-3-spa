<template>
  <div>
    ...
    <h4
      v-if="layout === 'default'"
      class="text-white mb-2 mt-1 fut-color-dynamic"
    >
      {{ rawOdds.name }}
    </h4>
    <p
    ...
    <div class="container-fluid table" style="padding: 0px 1px">
      <div class="row w-100 m-0">
        <div
          ...
          :class="[
            rawOdds.col ? 'col-' + rawOdds.col : 'col-3',
            'px-0', getLevarage(odds.leverage)
          ]"
        >
          <fut-button
            v-if="!odds.finished"
            ...
            :class="[
              ...
              hasSelected(event.id, odds.type, slugify(rawOdds.name)) ? layout === 'oldBet365' ? 'bet-selected-old' : 'bet-selected' : '',
              'ticket-odd-t'
            ]"
          ...
          >
            ...
            <span class="text-bet ms-2 position-relative">
              <span style="vertical-align: middle">
                <font-awesome-icon
                  v-if="odds.blocked" class="text-white flash"
                  :class="{ 'top-displacement': layout === 'FB' }"
                  style="font-size: 22px"
                  :icon="['fas', 'lock']"
                />
                <font-awesome-icon
                  v-if="odds.finished" class="text-white flash"
                  :class="{ 'top-displacement': layout === 'FB' }"
                  style="font-size: 22px"
                  :icon="['fas', 'times']"
                />
                <font-awesome-icon
                  v-if="odds.leverage && odds.leverage === 'up'"
                  class="text-white flash"
                  :class="{ 'top-displacement': layout === 'FB' }"
                  style="font-size: 22px"
                  :icon="['fas', 'caret-up']"
                />
                <font-awesome-icon
                  v-if="odds.leverage && odds.leverage === 'down'"
                  class="text-white flash-2"
                  :class="{ 'top-displacement': layout === 'FB' }"
                  style="font-size: 22px"
                  :icon="['fas', 'caret-down']"
                />
