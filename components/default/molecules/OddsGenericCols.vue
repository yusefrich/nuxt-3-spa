<template>
  <div :class="{'bg-dark-2 mb-01': layout === 'FB' || layout === 'FB2'}">
    ...
    <h4 v-else class="text-white mb-2 mt-1 fut-color-dynamic fut-color-dynamic">
      {{ rawOdds.name }}
    </h4>
    <div class="table-responsive" style="padding: 0px 1px">
      <table class="table" :class="{'mb-0': layout === 'oldBet365'}">
        ...
        <template v-for="index in getColsMaxLine(getOddsHeaderGroup(rawOdds.odds))" :key="`group_odds_${rawOdds.name}_${index}`">
          <tbody v-if="rawOdds.odds" class="stripped">
            <template v-for="(odd, j, k) in groupedOdds" :key="`group_body_odds_${rawOdds.name}_${j}_${k}`">
              <td
                :class="[
                  'position-relative',
                  'text-bet',
                  ...
                  {'fb-active' : (layout === 'FB' || layout === 'FB2') && hasSelected(odd[index - 1]?odd[index - 1].type:null)},
              ...
              >
                <div class="p-0" :class="{'d-flex m-1 mt-2 m-md-0 odd-flex-container': layout === 'FB' || layout === 'FB2'}">
                  <template v-if="layout === 'FB' || layout === 'FB2'">
                    <div v-if="odd[index - 1] && !odd[index - 1].finished" class="fb-side-width" />
                    <span v-if="odd[index - 1] && !odd[index - 1].finished" class=" text-start text-bet-header" :class="{'bg-primary-dark': font === 'dark'}">
                  ...
                  <template v-else>
                    <span v-if="odd[index - 1] && !odd[index - 1].finished" class="text-white fut-color-dynamic">
                      {{ odd[index - 1].desc ? odd[index - 1].desc : ' ' }}
                    </span>
                  </template>
                  <span v-if="odd[index - 1] && !odd[index - 1].finished" :class="[{'odd-fb ms-auto': layout === 'FB' || layout === 'FB2'}]">
                    <span v-if="odd[index - 1].super_odd && odd[index - 1].super_odd.slug === odd[index - 1].type" :class="[{'fb-super-odd-container': layout === 'FB' || layout === 'FB2', 'super-odd-container': layout !== 'FB'}]">
                    ...
                    <span v-else class="position-relative ms-odd-3" :class="{'ms-odd-0': layout === 'FB' || layout === 'FB2'}">
                      <span style="vertical-align: middle">
                        <font-awesome-icon v-if="odd[index - 1].blocked" class="text-white fut-color-dynamic flash" :class="{ 'top-displacement': layout === 'FB' || layout === 'FB2' }" style="font-size: 22px" :icon="['fas', 'lock']" />
                        <font-awesome-icon v-if="odd[index - 1].finished" class="text-white fut-color-dynamic flash" :class="{ 'top-displacement': layout === 'FB' || layout === 'FB2' }" style="font-size: 22px" :icon="['fas', 'times']" />
                        <span v-if="odd[index - 1].leverage && odd[index - 1].leverage === 'up'" class="leverage-up-arrow" :class="{ 'top-displacement': layout === 'FB' || layout === 'FB2' }" style="vertical-align: middle">
                          <font-awesome-icon class="text-white fut-color-dynamic flash" style="font-size: 22px" :icon="['fas', 'caret-up']" />
                        </span>
                        <span v-if="odd[index - 1].leverage && odd[index - 1].leverage === 'down'" class="leverage-down-arrow" :class="{ 'top-displacement': layout === 'FB' || layout === 'FB2' }" style="vertical-align: middle">
                          <font-awesome-icon class="text-white fut-color-dynamic flash-2" style="font-size: 22px" :icon="['fas', 'caret-down']" />
                        </span>
                      </span>
