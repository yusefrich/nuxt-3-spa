<template>
  <div v-if="currentEvent">
    <fut-button class="btn px-0" @click="$router.go(-1)">
      <p class="text-primary fw-bold mb-0">
        <font-awesome-icon class="me-3" :icon="['fas', 'long-arrow-alt-left']" />
        {{ $t('i18n_voltar') }}
      </p>
    </fut-button>
    <div class="position-relative">
      <div class="on-top">
        ...
        <div v-if="!live" class="">
          ...
          <p v-if="currentEvent.time" class="text-center text-white mb-0 fut-color-dynamic">
        ...
        <div v-else class="text-center d-flex justify-content-center">
          ...
          <template v-if="currentEvent.result">
            <div v-for="(result, j) in currentEvent.result" :key="'result_inplay_' + j + '_' + (result.home + result.away)" class="me-2">
              <p v-if="j === 0 && j+1 !== currentEvent.result.length" class="mb-0 text-white fut-color-dynamic">
                <small>Sets</small>
              </p>
              <p v-else-if="j+1 === currentEvent.result.length" class="mb-0 text-white fut-color-dynamic">
                <small>{{ $t('i18n_placar') }}</small>
              </p>
              <p v-else class="mb-0 text-white fut-color-dynamic">
                <small>{{ $t('i18n_jogos') }}</small>
              </p>
              <h4 :key="'home_result' + +result.home" class="text-orange mx-1">
                {{ result.home }}
              </h4>
              <h4 :key="'away_result' + +result.away" class="text-orange mx-1">
                {{ result.away }}
              </h4>
            </div>
          </template>
        </div>
        <p v-if="live && currentEvent.time" class="text-center mb-0 text-white fut-color-dynamic">
          <small> <font-awesome-icon class="icon me-2" :icon="['fas', 'clock']" /> {{ currentEvent.time }}</small>
        </p>
        <p v-if="currentEvent.datetime" class="text-center mb-0 text-white fut-color-dynamic">
          <small> <font-awesome-icon class="icon me-2" :icon="['fas', 'clock']" /> {{ formattedHour(currentEvent.datetime) }}</small>
        </p>
        <p v-if="currentEvent.datetime" class="mb-0 text-center text-white fut-color-dynamic">
          <small>{{ formattedDate(currentEvent.datetime) }}</small>
        </p>
      </div>
      <img v-if="live" class="bg-img-field" src="/field.jpg" alt="">
    </div>
  </div>
</template>
<script>
