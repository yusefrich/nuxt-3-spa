<template>
  <div>
    ...
    <h4 v-if="layout === 'default'" class="text-white mb-2">
      {{ rawOdds.name }}
    </h4>
    ...
    <div class="table-responsive">
      <table class="table" :class="{'mb-0': layout === 'oldBet365'}">
        ...
        <tbody v-for="(groups, index) in groupOddsInTableFormat(rawOdds.odds)" :key="`group_odds_${groups.name}_${index}`" class="stripped">
          ...
          <td
            v-if="groups.odds && groups.odds[2]"
            :class="[layout === 'oldBet365' ? 'table-td-old-bet' : 'table-td-dark', layout === 'oldBet365' ? 'bet365' : 'default', 'text-bet', layout === 'oldBet365' ? 'old-bet-click-bet' : 'click-bet', hasSelected(event.id, groups.odds[2].type, slugify(rawOdds.name))?layout === 'oldBet365' ? 'bet-selected-old' : 'bet-selected':'']"
            @click="()=>!groups.odds[2].odds||groups.odds[2].odds<=0?{}:$emit('selectBet', { gameId: event.id, odd: groups.odds[2], market: rawOdds.name, event: `${event.home_team} x ${event.away_team}`, validate: event.datetime, live: live })"
          >
            {{ groups.odds[2].odds ? groups.odds[2].odds : '-' }}
          </td>
        </tbody>
      </table>
    </div>
...
export default {
  ...
  methods: {
    ...
    groupOddsInTableFormat (odds) {
      ...
      chunks.forEach((e) => {
        odds.forEach((j) => {
          if (e.name === j.name && j.header === 'Exatamente') {
            e.odds.push(j)
          }
        })
      })
