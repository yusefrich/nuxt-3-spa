<template>
  <div>
    ...
    <div class="row px-2 m-0">
      <div v-for="(header, j) in getHeaders(rawOdds.odds)" :key="'oddColums_'+j" :class="[`col-12 col-md-${rows}`, 'text-white']">
        <p class="pt-2 ps-3 mb-2 text-white fw-bold" :class="{bet365: layout === 'oldBet365'}">
          {{ header }}
        </p>
      </div>
    </div>
    <div class="row px-2 m-0">
      ...
      >
        <p class="pt-2 ps-3 mb-2" :class="{bet365: layout === 'oldBet365'}">
          {{ odd.name }} <span class="text-bet">{{ odd.odds ? odd.odds : '-' }}</span>
        </p>
      </div>
    </div>
  </div>
