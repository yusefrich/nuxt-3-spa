  ...
  >
    ...
    >
        ...
        >
          <ul class="casino-categories-list">
            <li
              ...
              :key="idx"
            >
              <NuxtLink :to="localePath(`/casino/category/${item.slug}`)" class="c-link sidebar-btn transition">
                <font-awesome-icon class="icon transition" :icon="item.icon" />
                <span>{{ item.text || item.slug }}</span>
              </NuxtLink>
            </li>
          </ul>
        </sk-collapsed-list>
        ...
        >
          <ul class="casino-categories-list">
            <li
              ...
              :key="idx"
            >
              <NuxtLink :to="localePath(`/sport/${league.sport_id}/league/${league.id}`)" class="c-link sidebar-btn transition">
      ...
      <div class="sidebar-list">
        ...
        />
        <NuxtLink
          v-if="dynamicAffiliate"
          :to="localePath(dynamicAffiliate.nav_link)"
          class="sidebar-btn transition"
        >
          <img
            v-if="dynamicAffiliate.img"
            :src="dynamicAffiliate.img"
            alt="Icon"
            class="nav-icon-img"
          >
          <font-awesome-icon v-else class="icon transition" :icon="['fas', 'bezier-curve']" />
          <span>{{ dynamicAffiliate.nav_name }}</span>
        </NuxtLink>
        <NuxtLink
          v-else
      ...
      <div class="sidebar-list">
        <NuxtLink
          v-if="dynamicSuport"
          :to="localePath(dynamicSuport.nav_link)"
          class="sidebar-btn transition"
        >
          <img
            v-if="dynamicSuport.img"
            :src="dynamicSuport.img"
            alt="Icon"
            class="nav-icon-img"
          >
          <font-awesome-icon v-else class="icon transition" :icon="['fas', 'bezier-curve']" />
          <span>{{ dynamicSuport.nav_name }}</span>
        </NuxtLink>
        <NuxtLink
          v-else
      ...
      <div class="sidebar-list">
        <a
          v-for="(item, idx) in filteredDynamicLinks"
          :key="idx"
          :href="item.nav_link"
          class="sidebar-btn transition"
        >
...
import SkCollapsedList from '@/components/sk/molecules/SkCollapsedList'
...
export default {
  ...
  computed: {
    ...
    filteredDynamicLinks () {
      ...
      return this.dynamicLinks.filter(
        item => item.nav_link.replace(regex, '') !== '/affiliates' && item.nav_link.replace(regex, '') !== '/suporte'
      )
    },
    dynamicAffiliate () {
      if (this.dynamicLinks && this.dynamicLinks.length) {
        const regex = new RegExp(window.location.origin, 'g')

        const affiliates = this.dynamicLinks.find(
          item => item.nav_link.replace(regex, '') === '/affiliates'
        )
    ...
    dynamicSuport () {
      if (this.dynamicLinks && this.dynamicLinks.length) {
        ...
        const suport = this.dynamicLinks.find(
          item => item.nav_link.replace(regex, '') === '/suporte'
        )
